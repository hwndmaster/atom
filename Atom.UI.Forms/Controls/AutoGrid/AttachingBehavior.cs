...
public sealed class AttachingBehavior : Behavior<DataGrid>, IDisposable
{
    ...
    public static AttachingBehavior FindMeFor(DataGrid dataGrid)
    {
        return Interaction.GetBehaviors(dataGrid).OfType<AttachingBehavior>().First();
    }
    ...
    public void SetupItemsSource(CollectionViewSource collectionViewSource)
    {
        ...
        _disposer.Add(new GroupingBehavior(AssociatedObject, _autoGridBuildContext.Value, collectionViewSource).Attach());
        _disposer.Add(new FilteringBehavior(AssociatedObject, _autoGridBuildContext.Value, collectionViewSource).Attach());
    ...
    {
        // Grids without an explicitly assigned AutoGridBuilder (e.g. the Logs grid) never raise
        // OnAutoGridBuilderChanged, so the fallback to DefaultAutoGridBuilder has to be armed
        // here. The lazy defers the evaluation until the items source is known.
        ...
        TakeCareOfDisposablesBehavior.AttachMe(AssociatedObject);
